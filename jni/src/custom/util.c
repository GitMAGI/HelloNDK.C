...
char* getETA(struct timeval st, struct timeval et){
    ...
    uint32_t out_usecs = fabs(fmod(usecs, 1000));
    uint32_t out_msecs = fabs(usecs / 1000);
    uint32_t out_secs = fabs(fmod(secs, 60));
    uint32_t out_mins = fabs(secs / 60);
    uint32_t out_hrs = fabs(secs / 3600);
